  ...
  render() {
    ...
    const OptionItem = (props) => {
      ...
      return (
        <option
          selected={
            this.state.isSelected == props.optionItem.user_code ? "selected" : ""
          }
    ...
    const checkBoxFormat = (cell, row) => {
      return (
        <div>
          <input
            onChange={() => this.onDailyEmailChange(cell, row)}
            type="checkbox"
            ref="criticalData"
            defaultChecked={cell}
          />
    ...
    const editEnabledColumn = (cell, row) => {
      const test5 = (e) => {
        ...
        this.requestADMIN_ROLE_OPERATIONS_edit(
          this.props.store.userState.token,
          obj
        );
      };
